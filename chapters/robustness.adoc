...
Because for some implementations robustness can come at a performance cost, the `VK_EXT_pipeline_robustness` extension was added to allow a developers to request robustness on a per-pipeline stage basis.

At `VkPipeline` creation time the `VkPipelineRobustnessBufferBehaviorEXT` and `VkPipelineRobustnessImageBehaviorEXT` can set which robustness for the resources of the pipeline.
