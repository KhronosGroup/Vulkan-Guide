...
| n/a | limited templates | | full generics support
|====
...
It is important to be mindful that matrices in GLSL are column-major, while matrices in HLSL and Slang are row-major by default. This affects things like matrix construction.

image::{images}high_level_shader_language_comparison_matrix.png[high_level_shader_language_comparison_matrix.png]
...
Slang's multi-target compilation system allows you to write shaders that can be compiled for multiple graphics APIs from a single source file, with conditional compilation to handle API-specific differences where parameter blocks aren't a good fit.
