...
=== Khronos extension layers

Some extensions not available everywhere can be implemented as layers. These layers can be used where the corresponding extension is not available. By default, these layers will disable themselves if the underlying driver provides the extension.

  * link:https://github.com/KhronosGroup/Vulkan-ExtensionLayer/blob/main/docs/synchronization2_layer.md[`VK_LAYER_KHRONOS_synchronization2`]. This layer implements the `VK_KHR_synchronization2` extension. B

  * link:https://github.com/KhronosGroup/Vulkan-ExtensionLayer[`VK_LAYER_KHRONOS_timeline_semaphore`]. This layer implements the `VK_KHR_timeline_semaphore` extension.

  * link:https://github.com/KhronosGroup/Vulkan-ExtensionLayer/blob/main/docs/shader_object_layer.md[`VK_LAYER_KHRONOS_shader_object`]. This layer implements the `VK_EXT_shader_object` extension.

  * link:https://github.com/KhronosGroup/Vulkan-ExtensionLayer/blob/main/docs/memory_decompression_layer.md[`VK_LAYER_KHRONOS_memory_decompression`]. This layer implements the `VK_NV_memory_decompression` extension.
