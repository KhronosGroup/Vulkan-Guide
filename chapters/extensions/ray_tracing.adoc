...
payload structures between all traversal stages, ray attribute structures
from the traversal control shaders, and callable data structures for callable
shaders.

All three of these structures consume driver-managed memory, the quantity of
which may scale based on the size of the structures themselves, and the number
...
Whilst acceleration structures can be built on any Vulkan memory heap,
