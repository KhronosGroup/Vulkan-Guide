...
This chapter goes over how to link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#interfaces[interface Vulkan with SPIR-V] in order to map data. Using the `VkDeviceMemory` objects allocated from `vkAllocateMemory`, it is up to the application to properly map the data from Vulkan such that the SPIR-V shader understands how to consume it correctly.

In core Vulkan, there are 5 fundamental ways to map data from your Vulkan application to interface with SPIR-V:

  * xref:#input-attributes[Input Attributes]
  * xref:#descriptors[Descriptors]
  ** xref:#descriptor-types[Descriptor types]
  * xref:#push-constants[Push Constants]
  * xref:#specialization-constants[Specialization Constants]
  * xref:#physical-storage-buffer[Physical Storage Buffer]
...
.Note
====
More information can be found in the xref:./vertex_input_data_processing.adoc[Vertex Input Data Processing] chapter
====

== Descriptors

A link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#descriptorsets[resource descriptor] is the core way to map data such as uniform buffers, storage buffers, samplers, etc. to any shader stage in Vulkan. One way to conceptualize a descriptor is by thinking of it as a pointer to memory that the shader can use.

There are various link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#VkDescriptorType[descriptor types] in Vulkan, each with their own link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#descriptorsets-types[detailed description] in what they allow.

Descriptors are grouped together in link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#descriptorsets-sets[descriptor sets] which get bound to the shader. Even if there is only a single descriptor in the descriptor set, the entire `VkDescriptorSet` is used when binding to the shader.
...
The Vulkan Spec has a link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#interfaces-resources-storage-class-correspondence[Shader Resource and Storage Class Correspondence] table that describes how each descriptor type needs to be mapped to in SPIR-V.

The following shows an example of what GLSL and SPIR-V mapping to each of the link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#descriptorsets-types[descriptor types] looks like.

For GLSL, more information can be found in the link:https://www.khronos.org/registry/OpenGL/specs/gl/GLSLangSpec.4.60.pdf)
...
----
// VK_FORMAT_R32_UINT
layout(set = 0, binding = 0, r32ui[GLSL Spec - 12.2.4. Vulkan Only: Samplers, Images, Textures, and Buffers] uniform uimage2D storageImage;
...
**Note:** `BufferBlock` and `Uniform` would have been seen prior to link:https://github.com/KhronosGroup/Vulkan-Guide/blob/master/chapters/extensions/shader_features.adoc#vk_khr_storage_buffer_storage_class[Vulkan 1.1 or VK_KHR_storage_buffer_storage_class]
...
link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#pipelines-specialization-constants[Specialization constants] are a mechanism allowing a constant value in SPIR-V to be specified at `VkPipeline` creation time. This is powerful as it replaces the idea of doing preprocessor macros in the high level shading language (GLSL, HLSL, etc).
...
With all the above examples it is important to be aware that there are link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/html/vkspec.html#limits[limits in Vulkan] that expose how much data can be bound at a single time.
  ...
  ** `VkPhysicalDeviceDescriptorIndexingProperties` if using xref:./extensions/VK_EXT_inline_uniform_block.adoc[Descriptor Indexing]
  ** `VkPhysicalDeviceInlineUniformBlockPropertiesEXT` if using xref:./extensions/VK_EXT_inline_uniform_block.adoc[Inline Uniform Block]
  * Push Constants
