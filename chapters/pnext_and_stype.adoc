...
The `void* pNext` is used to allow for expanding the Vulkan Spec by creating a Linked List between structures. The link:https://www.khronos.org/registry/vulkan/specs/1.3/html/vkspec.html#fundamentals-validusage-pNext[pNext valid usage] section of the Vulkan Spec goes into detail explaining the two different variations of `pNext` structures. The `VkStructureType sType` is used by the loader, layers, and implementations to know what type of struct was passed in by `pNext`. `pNext` is mostly used when dealing with extensions that expose new structures.
