...
An application submits work to a `VkQueue`, normally in the form of `VkCommandBuffer` objects or xref:./sparse_resources.adoc[sparse bindings].
...
The following are the queue operations found in link:https://www.khronos.org/registry/vulkan/specs/1.2-extensions/man/html/VkQueueFlagBits.html[VkQueueFlagBits]:
  ...
  * `VK_QUEUE_SPARSE_BINDING_BIT` used for binding xref:./sparse_resources.adoc[sparse resources] to memory with `vkQueueBindSparse`.
  * `VK_QUEUE_PROTECTED_BIT` used for xref:./protected.adoc[protected memory].
