...
The first thing to check is if your xref:./platforms.adoc[platform] even supports Vulkan. Each platform uses a different mechanism to manage how the xref:./loader.adoc[Vulkan Loader] is implemented. The loader is then in charge of determining if a Vulkan Driver is exposed correctly.
