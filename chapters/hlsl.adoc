...
Vulkan does not directly consume shaders in a human-readable text format, but instead uses link:./what_is_spirv.md[SPIR-V] as an intermediate representation. This opens the option to use shader languages other than e.g. GLSL, as long as they can target the Vulkan SPIR-V environment.
