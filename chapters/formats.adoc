...
link:https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/vkspec.html#formats-packed[Packed formats] are for the purposes of address alignment. As an example, `VK_FORMAT_A8B8G8R8_UNORM_PACK32` and `VK_FORMAT_R8G8B8A8_UNORM` might seem very similar, but whe using the formula from the link:https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/vkspec.html#fxvertex-input-extraction[Vertex Input Extraction section of the spec]
