...
Vulkan is supported on the link:https://fuchsia.dev/fuchsia-src/concepts/graphics/magma/vulkan[Fuchsia operation system].

== iOS

Vulkan is not natively supported on iOS, but can still be targeted with xref:./portability_initiative.adoc[Vulkan Portability Tools].
...
Vulkan is not natively supported on MacOS, but can still be targeted with xref:./portability_initiative.adoc[Vulkan Portability Tools].
