...
For simplicity, there are 3 values in `VkPhysicalDeviceDescriptorHeapPropertiesEXT` to handle the general cases:

* `samplerDescriptorSize`
* `bufferDescriptorSize`
* `imageDescriptorSize`

If you want to pack descriptors even more tightly in memory, you can use `vkGetPhysicalDeviceDescriptorSizeEXT` to get the exact size.

For example, if a driver needs `16` bytes for a Uniform Buffer, but `32` bytes for a Storage Buffer, then `bufferDescriptorSize` will be `32` (the worst case size). By calling `vkGetPhysicalDeviceDescriptorSizeEXT(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)` you can get the exact size (`16` in this case) from the driver.
...
There 2 main restrictions are

1. There is no descriptors arrays allowed (because there is no defined math how that would work).
2. The descriptor types are limited (images and samplers are not allowed)

The the following examples, the new shader code looks like:
...
[NOTE]
====
Only Uniform Buffers are allowed for this mapping, as it must be read only.
====
...
This is similar to the previous example but it uses an extra level of indirection to read the descriptor from a buffer instead of inside the push data.

[NOTE]
====
Only Uniform Buffers, Storage Buffers, and Acceleration Structures are allowed for this mapping.
====
...
A `Shader Record`` refers to an entry in the link:https://docs.vulkan.org/spec/latest/chapters/raytracing.html#shader-binding-table[Shader Binding Table (SBT)]
[NOTE]

The mappings work the exact same, just instead of mapping to a descriptor, such as a buffer/image/accelerationStructure, it maps to a shader record.

- `VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_SHADER_RECORD_INDEX_EXT` works just like `VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT`
- `VK_DESCRIPTOR_MAPPING_SOURCE_SHADER_RECORD_DATA_EXT` works just like `VK_DESCRIPTOR_MAPPING_SOURCE_PUSH_ADDRESS_EXT`
- `VK_DESCRIPTOR_MAPPING_SOURCE_SHADER_RECORD_ADDRESS_EXT` works just like `VK_DESCRIPTOR_MAPPING_SOURCE_INDIRECT_ADDRESS_EXT`
...
> "Untyped" is not the official spec term. There is a new link:https://github.com/KhronosGroup/SPIRV-Guide/blob/main/chapters/descriptor_heap.md[SPV_EXT_descriptor_heap] extension that provides these heaps in SPIR-V, but to access them, you need to use SPIR-V untyped pointers. You could use `SPV_KHR_untyped_pointers` without `SPV_EXT_descriptor_heap` but it is not very uncommon.
...
One of the hardest issues making an application portable across vendors is mastering the art of alignment.
...
The main issue is the "single descriptor" needs to both belong in the Resource heap (for the sampled image) and the Sampler heap (for the sampler).

This means for the untyped pointer model, it is not possible at all to use a combined image sampler. This would require some way to set the array stride separately for both the sampled image and sampler, which was actively decided to not support. Therefor, if you want to tranisition to using untyped pointers, you will need to use seperate sampled images and samplers.

If you are using the binding mappings, you will see the `samplerHeapOffset` and `samplerHeapArrayStride` which allow you to bind the sampler portion.
...
Some mappings also have a `useCombinedImageSamplerIndex`, which is a way to pack the same index for both the sampled image and sampler.

For example, if using `VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT` instead of going

[source,c++]
----
VkDescriptorMappingSourcePushIndexEXT::pushOffset = 0;
VkDescriptorMappingSourcePushIndexEXT::samplerPushOffset = 4;

vkCmdPushDataEXT(offset = 0, size = 4, value = 6); // image offset to 6
vkCmdPushDataEXT(offset = 0, size = 4, value = 7); // sampler offset to 7
----

you now can pack it into a single 32-bit word.
...
// equation found in spec
uint32_t combined_index = (image_index & 0xfffff) | ((sampler_index & 0xfff) << 20);
vkCmdPushDataEXT(offset = 0, size = 4, value = combined_index);
----
