...
[NOTE]
====
`VK_KHR_synchronization2` の仕様では、`VK_KHR_create_renderpass2` と `VK_KHR_get_physical_device_properties2` を要件として挙げています。そのため、これらの拡張機能を使わずに synchronization2 を使用すると、検証エラーが発生する可能性があります。拡張機能の要件は再評価されており、これが完了すると検証が調整されます。
====
