...
Vulkan のマイナーリリースバージョンは、それぞれ異なる拡張機能をコアにlink:https://docs.vulkan.org/spec/latest/chapters/extensions.html#extendingvulkan-compatibility-promotion[昇格]させました。つまり、アプリケーションが少なくともその Vulkan バージョンを要求していれば（そのバージョンが実装でサポートされていれば）、昇格された機能を使用するために拡張機能を有効にする必要はなくなりました。
