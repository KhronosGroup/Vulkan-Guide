...
The Guide has been converted from Markdown to Asciidoc format. It is
possible to view the individual chapters (pages) as before, starting with
this README, or to generate a single document containing each chapter using
'make guide.html' with the 'asciidoctor' command-line tool installed.
